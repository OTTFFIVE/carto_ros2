...
// add set_slam server
  set_slam_server_ = node_->create_service<cartographer_ros_msgs::srv::SetSlam>(
      kSetSlamServiceName,
      std::bind(
          &Node::handleSetSlam, this, std::placeholders::_1, std::placeholders::_2));
...
bool Node::handleSetSlam(
  ...
  LOG(INFO) << "Received set slam cmd: " << request->cmd;
